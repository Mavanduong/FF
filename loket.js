// ==UserScript==
// @name         AutoHeadlockProMax v14.9.1-BurstHeadSnap-UltraInstant
// @version      14.9.1
// @description  Ghim đầu tức thì + 7-8 viên đầu bắn cùng tốc độ viên đầu tiên
// @match        *://*/*
...
const CONFIG = {
  tickIntervalMs: 0.1,
  crosshairNearThresholdPx: 2000, // xa hơn để khóa từ xa
  ...
  shakeAmplitudePx: 0,
  shakeNearFactor: 0,
  ultraBurstFirstShots: 8,     // số viên đầu ra cùng tốc độ viên đầu
  ultraBurstDelayMs: 0         // delay giữa các viên siêu nhanh (0 ~ 1ms)
};
...
function burstFire(count) {
  ...
  const burstLoop = () => {
    ...
    fireOnce();
    fired++;
    // Nếu vẫn trong 7–8 viên đầu thì bắn gần như đồng thời
    const delay = (fired <= CONFIG.ultraBurstFirstShots) ? CONFIG.ultraBurstDelayMs : 10;
    setTimeout(burstLoop, delay);
  };
  burstLoop();
}
function clampAimMove(current, target) {
  return target; // tức thì, không clamp
}
...
function applyShake(pos) { return pos; }
